...
embeddings_index = dict(get_coefs(*o.split(' ')) for o in open('embeddings/glove.840B.300d.txt'))
...
for train_index, test_index in kf.split(X_train['num_vars']):
    ...
    predict += model.predict(X_test, batch_size=1000, verbose=1) / num_folds
...
sample_submission.to_csv('submissions/submit_cnn_avg_' + str(num_folds) + '_folds.csv', index=False)
